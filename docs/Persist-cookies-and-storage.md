...
HTTP Set-Cookie 与 document.cookie 均在属性解析后应用最后一个有效 `Max-Age`，优先于 `Expires`，后续非法值不清除先前有效值。请求 Cookie 头与 document.cookie 的可见结果按 Path 长度降序、同长度按创建顺序排列；同键有效 Cookie 的替换保留原位置，删除或过期后重新创建取得新位置。内部快照和新版文件保留创建顺序。历史裸数组及缺少 creation_order 的 version 1 文件按文件数组顺序建立初始顺序，无法恢复旧文件未记录的真实创建历史；新格式保持 version 1，不改变对外 CookieInfo 字段。
