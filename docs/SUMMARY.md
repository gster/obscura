...
OB-021/OB-034 CDP outbound reservation 边界（前一切片）：每连接 outbound 保留队列为 1024 条、128 MiB 总 UTF-8 bytes、单消息 80 MiB；reservation 在正常 writer 路径覆盖从排队到 envelope 完成 socket send，单次写 deadline 为 10 秒。count/bytes overflow、writer I/O、writer timeout、connection shutdown 均首次原因 sticky close，并停止后续命令排队；服务不会靠静默丢弃、截断或脱敏消息维持连接。合法 `Browser.close` 先封闭新消息，再在同一 10 秒总预算内尝试 flush 已接受 envelope；写失败或超时则断开，reservation 释放本身不是送达确认。未覆盖序列化瞬时内存、inbound `ServerMessage`、`pending_events`、Host/Origin/auth 和任意同步 V8 执行期间的立即断连；整体 OB-021/OB-034 仍未完成。
