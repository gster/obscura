...
三层逻辑 payload 预算不是总 RSS 上限；本轮也没有覆盖完整断连组合矩阵、仅等待 `commands.recv()` 的 idle Worker 被 cancellation 主动唤醒、admission 前 TCP/kernel 分配、容器 capacity、Tungstenite/TCP、上游 observation queue 和响应正文，因此 OB-034 保持未关闭。
