...
进展（2026-09-20，native input 切片；2026-09-22 补充 maxlength 资格）：CDP move/press/release 已通过 typed Page API 接入共享 native hit-test、受保护事件分发、焦点与默认动作；移除该分支的页面 JS 拼接和全局按压状态。官方 Playwright 六组离线鼠标场景已与 Chrome 对照通过；document.open/导航使原按压状态失效，no-render 坐标输入明确 unsupported。wheel 已另行接入 typed Page API，九组 Chrome 对照覆盖嵌套滚动、取消、页面覆盖和参数边界；滚动队列按文档 epoch 隔离，hidden overflow 不接受用户 wheel。keyboard/text 现通过 `Page::dispatch_keyboard_input` / `Page::insert_text` 接入受保护事件和原生编辑，八组 Playwright CDPSession 场景与 Chrome exact 对照覆盖选择区、阶段、取消、metadata、公开 API 覆盖、focus/document 重入、协议错误，以及普通 input/textarea maxlength 用户编辑的 17 个子场景。maxlength 按 UTF-16 容量且不拆 Unicode scalar，beforeinput 后重新读取 value/selection/attribute，保留 insertText 与 key-text 的实测 caret 差异，并覆盖 CR/LF、Enter、动态 grow/shrink、超长脚本值删除与官方 Locator.fill；每个事件的瞬时 value/selection/maxlength 和完整原始协议/fixture 字节均保留。真实待处理导航另由 Rust 回归覆盖；该阶段尚未资格化的 contenteditable 已由下述有界切片补充，IME/composition、grapheme/word 编辑、任意命令、平台快捷键默认动作和复杂表单默认动作仍未资格化。等待、资源观察、限额及启动保护的后续归口仍待完成，本项保持未关闭。

进展（2026-09-22，contenteditable 有界文本输入资格）：提交 `195be5537f8da41f7c0235318b6090e57eef4b19` 将 `Input.insertText` 与带 `text` 的 `Input.dispatchKeyEvent` 接入 focused editing host。资格仅覆盖同一 Text 节点内的折叠选区或选择替换、嵌套 inline DOM 保留、可取消且 trusted 的 `beforeinput`、mutation 后 trusted `input`、handler append-only DOM 重入、`contenteditable=false` island 的 beforeinput-only 行为、focus 转移，以及一个严格限定的空文本删除形状：host 的 direct `span` 仅含被全选 Text，左右均为 Text sibling；该形状删除 wrapper、合并文本并折叠 caret。原生 mutation 路径按 observer options 生成独立 record，支持 oldValue、disconnect、精确 removed-node sibling，并不调用页面覆盖的 `parentNode`、`isConnected` 或 `querySelectorAll`。跨节点 selection、未满足上述条件的空字符串整节点结构删除，以及 Backspace/Delete/Enter 明确返回 `INPUT_CONTENTEDITABLE_UNSUPPORTED`；paragraph split、IME/composition、grapheme/word editing 保持未资格化。Chrome 152/Obscura paired 的 8 个场景通过；工具保留完整 DOM/event/selection path、fixture、协议和进程流，仅 comparator 投影本资格字段。本项仍未关闭。

进展（2026-09-22，`Input.setIgnoreInputEvents` 资格）：每个 attached CDP session 持有独立 contribution，同一 target 以 live session OR 生效；`false` 只清调用方，导航保留，detach/target close 清理，新 attachment 默认 false，其他 target 独立。有效 ignore 在参数校验后抑制 mouse/wheel/key，不进入 Page/V8、input navigation 或命令型 screencast；`Input.insertText` 仍执行。九组 Chrome 152/Obscura paired 场景和 Rust 回归覆盖 true/false、双 session、导航、detach/reattach、跨 target、参数与 session 错误；所有 raw 字段保留。恢复阶段不把既有 click/`which`/coordinate caret 差异冒充本 gate 资格；`dispatchTouchEvent`、完整 mouse parity、通用 contenteditable/rich editing 与 IME 仍未资格化，上述有界同 Text slice 除外；no-render 实际输入仍 unsupported。本项保持未关闭。

进展（2026-09-23，network-idle truthfulness 子切片，提交 `75972fa7877d0ce29663baec01c73ef9df1270a4`）：Page/frame/Worker 共用 generation-scoped `NetworkActivityTracker`；`networkidle0` 与 `networkidle2` 按 0/2 active requests 及真实 500ms quiet window 判定。旧 document 的迟到请求 start/terminal 不污染新代。等待超时返回 `NetworkIdleTimeout` 而不伪报 idle；CDP 不再将 DCL/Load 直接投影成 `networkIdle`，commit 后超时仍先同步真实 loader/context 与实际到达的 lifecycle，再返回错误。历史 204/205 导航保留旧文档/loader，并完整投影请求、response、`ERR_ABORTED` terminal 与 `frameStoppedLoading`。

定向 release/render 回归 **4/4**，工具 unittest **140/140**；release/render 全工作区降低 nextest 并发至 2 后 **2389/2389**、4 skipped。默认并发两轮的同一 MCP loopback fixture 连接失败与 **1/1** 精确重放均保留，低并发全量通过；不将重跑称为源码修复。提交后显式固定 `OBSCURA_VERSION=0.1.0-dev+75972fa` 的 no-render/render exact CLI build、network-idle 五场官方 Playwright smoke、常规 smoke + **37-method** profile、三项 migration gates 及固定 benchmark obstacle **33/33** 通过。最终 no-render/render binary SHA-256 分别为 `3e2faa8536b695aff8efb4e9eb2f2df95c378fe2532a4179170835b0c89bd343`、`7dea164cca98e7def3a50a0b781800f5c3a4dcff53dcb7e861e6df3bea69f0db`。Chrome **152.0.7977.85** 同 fixture 的 network-idle/timeouts/204 abort 对照通过；静态 idle 509.9ms、delayed 2016.4ms、never timeout 1004.5ms、document.open 960.0ms、204 abort 3.2ms。该 Chrome 对照不声称 document.open 后完整 DOM 语义相同。Astra light 终审 **0 blocker/0 major/0 minor/0 nit**。

baseline/candidate 以相同 persona、fixture、视口与 capture path 交错运行三轮。静态 idle 中位数为 **601.9ms/600.2ms**，在约 ±10% 噪声内，且候选未扩大相对 Chrome 的既有约 92ms 差值；含 1.5s fetch 的 delayed case 为 **512.7ms/2024.2ms**，差值是等待真实 response 加 quiet window，接近 Chrome 的 2016.4ms；never case 从基线 false success 改为候选与 Chrome 一致的 timeout，204 从基线约 4005ms timeout 改为候选与 Chrome 一致的约 5.6ms abort。初次 obstacle 调用遗漏必填 persona 的 0/33 配置失败也完整保留；提交版在固定 `windows_chrome145` persona 下 33/33。Cargo target 清理释放 **15.8 GiB**，本轮新建的工具 venv 也已移除；独立二进制和所有 raw evidence 保留在 `/private/tmp/ob021-network-idle-final-build/`。该子切片不关闭 OB-021。
...
进展（2026-09-21，passive callback ownership 切片）：Page、iframe 与 Dedicated Worker 共用的 `CallbackRegistry` 不再用会静默失败的 `try_write` 注册或移除观察者。注册表以短时 mutex 保护 immutable `Arc` snapshot；dispatch 在调用用户 callback 前释放锁，保持登记顺序，并规定 dispatch 中的 add/remove 从下一条 observation 生效。移除的 callback 及其 capture 在锁外析构，避免析构重入注册表死锁。公开 callback 签名、ID 和顺序保持不变，`RequestInfo`、`Response`、body、兼容 headers 与原始 header bytes 原样传递，不脱敏、不删字段。回归覆盖 dispatch 中注册、自移除、callback capture 析构重入，以及完整 binary body、Cookie、Authorization、重复与非 UTF-8 header；最终 `obscura-net` **158/158**，相关 Page/Worker render **5/5**、no-render Worker **1/1**，根 release/render nextest **2227/2227**，4 skipped，两种 exact CLI release build 和固定 benchmark 障碍课 **33/33** 均通过。Astra light 首审指出移除时在 mutex 内 drop 最后一个 callback Arc 会因 capture 析构重入而死锁；移至锁外并增加有界回归后，复审无 blocker、major 或 minor。该切片不处理 callback panic 隔离、CDP 多 session `Network.enable` 订阅/fanout、上游 JS/Worker 4096 oldest-drop、持久 observation history 或请求正文保留；本项保持未关闭。

进展（2026-09-21，CDP multi-session Network ownership 切片）：`Network.enable` 现只为调用它的有效 Page session 建立订阅，`requestWillBeSent`、`responseReceived`、`loadingFinished`/`loadingFailed` 按事件产生时已启用的 session fanout；请求开始时的订阅快照单独决定该 requestId、redirect body alias 与 Document loader alias 的 `Network.getResponseBody` 资格。晚启用者可收到之后产生的终态但不能读取启用前开始的正文；`Network.disable`、detach、Page close 和连接清理只撤销对应 session，重启用不会恢复旧资格。Fetch pause 仍由独立的单 owner 路径控制，Fetch-only session 不自动订阅 Network；`Network.getResponseBody` 使用 Network requestId，不接受 Fetch `intercept-N` 作为替代。sessionless enable/disable 保持兼容 no-op，不能清空兄弟 session 或共享 Fetch body store。store-wide body budget failure 用每 session 单一 capability 表示，不为每个失败 ID 无界增长，并且不能解锁已有 retained body。所有事件、raw headers 和 body 原样送达，不脱敏、不删字段。Chrome 152 原始对照和官方 Playwright 1.60.0 双 session 实测覆盖早/晚订阅、禁用/重启用、共享 requestId、独立 body 可见性与 Fetch/Network 分离；Astra light 终审无 blocker、major 或 minor。尚未收敛上游 JS/Worker 4096 oldest-drop、持久 observation history、Page body budget 与 Chrome per-agent 参数差异、普通非拦截请求 start emission 的统一架构，以及 Fetch stream 的共享消费；本项保持未关闭。

进展（2026-09-21，request-body retention 切片）：新增 Page-owned 独立 request-body store，按完整原始 bytes 保留 JS fetch/XHR、Request、module、Dedicated Worker、Page 原生表单导航与逐 hop redirect 的 standard/transport body；2 MiB 后 spool，默认 256 MiB unique bytes/16384 canonical entries，首次预算或 I/O 失败 sticky，并在发送前拒绝后续需要新增正文 capture 的请求；无正文请求不受该正文预算影响。显式空与缺省严格区分；相同 body 可共享 raw storage，canonical entry 仍单独计数；bodyless alias 不累积 tombstone。302/303 清 body，307/308 保留；Fetch override 前后 body 各有稳定 per-hop ID。CDP 事件、`Network.getRequestPostData` 与 MCP 投影均可读取完整 UTF-8 或 base64 binary body，并服从请求开始时的 Network session ownership；原生 redirect 链按 hop 顺序复用同一 loader ID。回归覆盖 0..255 bytes、empty/absent、redirect/failure/teardown、Worker、双 session、100000 bodyless 请求和预算 fail-before-send；根 release/render nextest **2255/2255**，4 skipped，两种 exact CLI build、固定 benchmark **33/33**、官方 Playwright 1.60.0 E2E/smoke 与 37-method profile 均通过。Astra light 终审为 0 blocker/major/minor。`Network.enable.maxPostDataSize` per-agent 语义、持久 observation history、普通非拦截资源 start/Page budget 统一和 Fetch stream 多消费者仍未完成；本项保持未关闭。

进展（2026-09-21，scripted Network observation admission 切片）：Page、JS runtime、module loader 与全部 Dedicated Worker 现共享一个有界 observation admission budget：最多 4096 条、64 MiB 完整序列化 metadata、单条 16 MiB，batch 全有或全无。已接纳记录持有可转移 reservation，在 active、teardown 与 Page drain 间移动不重新计数；Worker 网络记录不再经过可丢弃的通用 Worker event channel，而是按 Worker 生命周期顺序直接进入 Page 共享 teardown FIFO。`ObscuraState::drop` 负责初始化失败、取消和 runtime 退出时的最后回收。首个 count/bytes/single-event/serialization overflow 为 sticky terminal：生产者立即拒绝后续 scripted network 工作，消费者在所有已接纳 reservation 释放且 accepted prefix 已进入 Page 后才看到失败，不能靠 overflow marker 继续丢旧事实。CDP 对每个已启用 Page session 在 accepted events 后发送一次 Obscura 扩展 `Network.observationFailed`，晚启用 session 仍可收到；MCP 保持正常 `{events:[...]}` 形状，仅失败时增加完整 `terminal_failure`。Cookie、Authorization、重复与非 UTF-8 raw header bytes 参与完整字节预算并原样保留，不脱敏、不删字段。该切片只替换上游 JS/Worker 4096 oldest-drop，不把 active `Page.network_events` 变成持久历史，也未完成请求正文、普通非拦截资源 start emission、Page body budget/per-agent 参数或 Fetch stream 多消费者契约；本项保持未关闭。

进展（2026-09-21，context-owned persistent Network history 切片）：每个 `BrowserContext` 现拥有 append-only `NetworkHistory`，以全局 sequence 和永不复用的 page-instance ID 保留跨导航、Page 关闭和多 Page 的完整 observation metadata、raw request/response headers，以及精确 request/transport-request/response body 引用。默认限制为 4096 条记录及 page instance、64 MiB metadata、16 MiB 单条、512 MiB body、32768 body entries、640 MiB persistent journal；失败保留 accepted prefix 并成为 context-wide sticky terminal，现有 sibling runtime/Worker/native producer 都停止后续网络工作。`serve`/`mcp --storage-dir` 使用 versioned manifest 与 checksummed length-framed journal，frame 完整写入并 `sync_data` 后才接纳；恢复只承认校验前缀并结构化报告不确定尾部。MCP 新增 histories/history/body 三个 bounded retrieval 工具，既有 requests 工具改读该 authority；CDP 新增 browser-level `Obscura.getNetworkHistories`、`Obscura.getNetworkHistory`、`Obscura.getNetworkBody`，并独立暴露 recovery failures。回归覆盖导航/关闭/多 Page、crash/corruption/预算收紧恢复、精确 binary body、producer/close/registration failure、原生 redirect response body 与 response-stage Fulfill 替换正文；根 release/render nextest 最终 **2277/2277**，4 skipped，两种 exact CLI build、固定 benchmark **33/33**、官方 Playwright 1.60.0 smoke 与 37-method profile 均通过。Astra light 终审为 0 blocker/major/minor。普通非拦截资源 start emission、Page body budget 与 Chrome per-agent `Network.enable.maxPostDataSize`、Fetch stream 多消费者仍未完成；本项保持未关闭。

进展（2026-09-21，ordinary native Network lifecycle 与 per-agent postData 投影切片）：Document、Stylesheet、classic Script 及 render Image/Font 的 transport 现统一在已知最终 prepared raw request headers/body 后、首次发送前同步接纳 Started，并以同一 logical request ID 产生 exactly-one Redirect/Finished/Failed terminal；每个 redirect hop 保留完整响应 headers/body，策略拒绝与 20-hop 上限不会先误报成功 redirect。普通请求先写 context-owned persistent history，再排入 Page/CDP live view；导航期间用共享 Notify 实时 drain，跨批次保留 `redirectResponse`，retired generation 不泄漏状态。Page response-body store 超预算仍显式失败，但 persistent history 可按自身预算保留生产者已完整取得的原始 body。render Page close 先关闭新 start、取消活动 trace 并同步落入 `Aborted` terminal；barrier 回归覆盖 Started 已被 observer 接纳但内部 start 状态尚未提交的并发窗口，关闭后不能继续发送或晚写 history。

`Network.enable.maxPostDataSize` 现按有效 Page session 独立保存，并按 UTF-8 原始字节数只投影标准 `requestWillBeSent.request.postData`、`postDataEntries` 与 Obscura 的 `postDataIsByteString`；canonical request/transport body、raw headers、Fetch pause 和 `Network.getRequestPostData` 保持完整。省略、null、0、负数为不限，正整数按 exact threshold，浮点/字符串拒绝，未知字段兼容接受；重复 enable 只改变未来事件投影，disable 清理该 session 的参数和读取资格。Chrome 152 原始探针确认这些边界。最终受影响 crate release/render nextest **1475/1475**，3 skipped；根 release/render nextest **2293/2293**，4 skipped（另有一个既有 render 测试被 nextest 标为 leaky 但通过）；两种 exact CLI build、固定 benchmark **33/33**、官方 Playwright Python 1.60.0 smoke 与 37-method profile 均通过。Astra light 三轮复核推动修复 redirect 失败终态、跨批次 redirectResponse、render close terminal 和 start/close 窄竞态，终审为 0 blocker/major/minor。首次障碍课漏传必填 persona 得到 0/33 的配置失败证据完整保留，随后按固定 `windows_chrome145` 输入通过。本切片不完成 Fetch stream 多消费者语义，OB-021 保持未关闭。

进展（2026-09-21，Fetch response stream shared-consumption 切片）：canonical response body、Network/Page 读取、persistent history 与 Fetch IO stream 共享 immutable raw backing；`Fetch.getResponseBody` 可重复调用，但与同一 canonical slot（含 alias）的一次 `takeResponseBodyAsStream` 互斥，stream 不再让 `Network.getResponseBody` 丢失正文。Fulfill replacement 产生新 generation，Network 读取新 body，已打开 stream 保持旧 body。Fetch handle 按 target 授权，同 target sibling flattened session 可顺序读取/关闭，其他 target 拒绝；disable、导航和 Page store clear 保留 handle，last detach、target/Page close、context/connection teardown 回收；PDF exact-session 生命周期不变。body-store guard 原子覆盖长度、IO admission 和 Fetch 状态转换，commit 校验实际长度；容量拒绝不消费正文或 handle。Chrome 152 原始探针确认 get/take 互斥、sibling/disable/last detach 边界，并确认未解除 response pause 时 Network body 仍不可读。首轮 Astra 发现并推动修复 replacement/reservation 竞态，首轮全 CDP 发现并修复 PDF 误清理；终审 0 blocker/major/minor。最终 CDP **359/359**、根 release/render nextest **2296/2296**（4 skipped），两种 exact CLI build、固定 benchmark **33/33**、官方 Playwright 1.60.0 smoke 和完整 37-method profile 均通过。原始 body、binary bytes、headers、协议与失败证据完整保留，不脱敏、不删字段。该 stream 边界完成，OB-021 的其他迁移项仍未关闭。
...
当前 transport 边界：inbound、pending events 和 outbound 分别有独立的 1024 条/128 MiB 预算；单项上限依次为 64 MiB text、80 MiB event、80 MiB outbound message，WebSocket 单 frame 为 16 MiB。inbound reservation 持续到命令执行或 deferred 结束；pending event 按完整序列化 envelope 精确计数且批量原子 admission；outbound reservation 持续到 socket send 完成，单次写最多 10 秒。overflow、I/O、timeout、unsupported binary message 和 connection shutdown sticky close 并停止后续命令排队，服务不会靠静默丢弃、截断或脱敏维持连接。合法 `Browser.close` 只对已接受 outbound envelope 做 10 秒有界 flush。CDP discovery 与 WebSocket upgrade 现共用严格 byte-level HTTP admission：精确路由、单一且允许的 Host、缺省允许 native client 无 Origin但拒绝跨站 Origin、可选静态 Bearer，以及 4096-byte request-head 上限。默认 loopback 保持无 token 兼容；non-loopback 启动默认要求显式 Host allowlist 与 token，危险豁免必须明确选择。拒绝发生在 WS handoff、live slot 和 V8 之前；SSRF `--allow-private-network` 不参与入口决策。

本轮同步 V8 断连切片将 WebSocket I/O 留在 server runtime，把 connection-local CDP processor/V8 保持在专用线程。Close、EOF/FIN、RST、writer failure、server shutdown 或 I/O task cancellation 一经该 I/O 路径观测，都会触发 per-connection sticky cancellation；注册表只中断当前 active isolate，并在每次 runtime enter/poll 与 termination clear 后重新检查关闭状态，从而覆盖当前 Page、后续导航新建 runtime、共享 parent runtime 的 iframe，以及由该 Page 创建的 Dedicated Worker runtime。连接输入关闭后先给 processor 1 秒清理 Fetch pause/page，仍未完成则中止异步 navigation/network wait；connection handler 被取消时也会中止独立 writer，不遗留 detached socket task。watchdog guard 的 Drop 会移除尚未触发的 generation，避免已取消调用稍后误杀复用 runtime。真实 WebSocket 回归覆盖无限 `Runtime.evaluate`、inline `while(true)` 导航和 stalled transport 导航：断开后 `max_connections=1` slot 均释放，第二连接可执行 `42`；另有 sticky cancellation、runtime churn、watchdog drop 和 handler cancellation 单测。断连不是事务回滚，终止前已经发生的 DOM、Cookie 或网络副作用可以保留；被关闭连接不重放 queued/deferred 命令。客户端只取消本地 asyncio wait 而不关闭 wire 时，服务端不会把它当作断连；FIN/RST 尚未被内核/reader 观测前也不宣称已经即时终止。

本轮直接断连资格切片新增两个选择性组合，不外推完整 iframe/Worker x Close/FIN/RST 矩阵。iframe + 原始 FIN 回归从子 frame 第一轮同步循环内发出唯一 console marker，证明实际 `FrameRealm` 已进入无限脚本；FIN 仅关闭客户端写半边并持续保留、排空读半边直到服务端关闭，避免未读数据导致 drop 被内核转换为 RST。断连前已排队的第二条命令同时包含同步 console marker 与独立 fetch fixture；两者均未出现，证明关闭连接没有执行或重放该命令，随后新连接仍能执行 `42`。Dedicated Worker + 原始 RST 回归用 linger-zero abortive close，并在 worker 第一轮同步循环内 `postMessage("started")` 后才断开；另一个 owner-alive 单测在 parent runtime 和 `WorkerRegistry` 仍存活时触发共享 cancellation，确认 `WorkerLease` 从 1 降至 0，而不是靠 Page/owner drop 间接终止。Worker loop 在 sticky cancellation 后退出，不再把 V8 termination 当普通错误无限重试。最终 render/no-render 断连回归均为 5/5，owner-alive Worker 定向回归在两种 feature 下均为 1/1，Worker 广义定向回归为 38/38；根 release/render nextest 为 2224/2224，4 skipped，唯一 leaky 是既有无关 `child_frame_tree::isolated_worlds_are_owned_by_their_exact_frame`；两种 exact CLI release build 成功，固定 benchmark 障碍课为 33/33。Astra light 复审确认 FIN/RST 资格和 no-replay 证据的 major 均已关闭，无新增 blocker、major 或 minor。

后续直接客户端断连矩阵切片把 WebSocket Close、原始 FIN 和 linger-zero RST 分别与 main、iframe、active Dedicated Worker 组成 3 x 3 真实 wire 矩阵。main 与 iframe 在同步无限循环第一轮写出完整 console marker，Worker 在第一轮 `postMessage("started")` 后才断开；Close 明确确认 frame 已发送并保留、排空旧 socket，FIN 保留读半边，避免测试自身把预期输入退化成 EOF/RST。每一组合都在 `max_connections=1` 下要求新连接取得 slot 并执行 `42`。connection cancellation 同时新增 sticky watch 广播；每个 Worker 保留一个 receiver，idle `commands.recv()` 与带远期 timer/I/O 的 autonomous event-loop wait 都可被主动唤醒，收到命令后再次检查 sticky 状态，非 idle 分支继续公平调度 command 与 autonomous progress。两个 owner-alive 精确回归先观察 Worker 确实停在对应 wait，再要求 lease 在一秒内从 1 降至 0，不依赖 owner drop。最终断连专项 render/no-render 均为 8/8，Worker 广义定向两种 feature 均为 43/43；no-render 中 `offscreen_webgl_owns_its_context_in_window_and_worker` 被 nextest 标记为 1 leaky，测试仍通过，本轮不宣称已解释或修复该退出期资源观察。根 release/render nextest 最终为 2300/2300，4 skipped。前两次全量各出现一个不同的既有 MCP loopback fixture 偶发失败，源码未改的单项重放均通过，最终全量通过，不把它们称作本轮修复。两种 exact CLI build、固定 benchmark 33/33、官方 Playwright Python 1.60.0 smoke 与完整原始协议 37-method 校验均通过。Astra light 首审指出并推动修正非 idle `select!` 饥饿风险和 Close 测试可能退化为其他断连输入，终审为 0 blocker、0 major、0 minor。

本轮 server-side terminal-source 切片把 writer I/O failure、writer timeout、server shutdown 和 outer I/O task cancellation 接入同一 connection-local sticky cancellation。生产 writer helper 在受控 `Sink` 的 I/O/timeout 失败后封闭 outbound、终止已进入同步无限循环的独立 V8，并释放 reservation；outer I/O task cancellation 与 sticky server shutdown 则通过完整 connection 路径中止 active main-realm V8，回收 `max_connections` slot，且 shutdown 会唤醒空闲 accept thread。全局 shutdown 使用 sticky watch；accept thread 通过 Mio listener readiness 与 OS waker 同时等待，取消先发布状态再唤醒，注册晚于取消也不会丢信号。单轮最多 accept 256 个，但未观察到 `WouldBlock` 时下一轮不会重新阻塞 Poll，避免 edge-triggered backlog 尾部等待 1 秒兜底；同步 HTTP discovery 设置 1 秒 socket 读写超时，拒绝路径设置 100ms socket 读写超时。最初 10ms polling 候选在 200 轮交替 `/json/version` 探针中产生 5.07 倍 median 回退，已弃用；最终 event-driven 候选相对 `757e8a0` 基线的 median 为 1.763ms 对 1.994ms（0.884 倍），p95 为 2.050ms 对 2.251ms。

该切片聚焦 render/no-render 均为 7/7；最终 render CDP 为 367/367、3 skipped，no-render 在排除既有 render-only `input_key_event_escaping` binary 后为 304/304、3 skipped；原始 no-render 全量的 6 个 `INPUT_UNSUPPORTED_WITHOUT_RENDER` 失败保留，不报告为全绿。根 release/render 最终 2307/2307、4 skipped；首轮根门禁的两个既有 MCP loopback fixture 偶发失败在源码未改的最终全量中未复现，不称为修复。两种 exact CLI build、固定 benchmark 33/33、官方 Playwright Python 1.60.0 smoke 与完整 37-method profile 均通过。Astra light 首审指出并推动修复 Mio batch 未 drain 到 `WouldBlock` 就重新 Poll 的风险，复审为 0 findings。writer 两项使用生产 writer helper、受控 `Sink` 与 active standalone V8，并非真实 TCP writer 故障注入；server shutdown 与 outer-I/O 两项只资格化 main realm，尚未覆盖 iframe/Worker server-side 矩阵。一次 320 并发 discovery 测试与既有 256 silent-pending 上限耦合而出现空响应，已删除该不可靠测试，不据此宣称 backlog burst 资格。三层逻辑 payload 预算仍不是总 RSS 上限；admission 前 TCP/kernel 分配、容器 capacity、Tungstenite/TCP、上游 observation queue 和响应正文也未纳入该预算，因此 OB-034 保持未关闭。

进展（2026-09-22，真实 writer terminal-source 与 Mio backlog 控制流切片）：生产 WebSocket/Tungstenite writer 保持 10 秒发送与 `Browser.close` flush deadline，新增的私有 connection I/O policy 只允许单测缩短 deadline 和被动观察归一化终态，不暴露 CLI、环境变量或公共 API。真实 loopback TCP 回归在 active Dedicated Worker 已用 `postMessage` 证明进入同步循环后，以 server socket `shutdown(Write)` 精确制造写半边 I/O failure；另一项把双方 TCP buffer 调小、客户端停止读取并发送 8 MiB 合法响应，以 250ms 测试 deadline 制造真实 backpressure timeout。两项都经过未替换的生产 WebSocket writer，并同时断言 `WriterIo`/`WriterTimeout` terminal reason、完整 writer 日志、socket 关闭和 connection slot 回收；8 MiB 低于 80 MiB 单消息及 128 MiB总预算，不能由 overflow 冒充。原受控 `Sink` 两项保留并明确命名为 unit helper，不再作为 wire 资格。

outer I/O task cancellation 与 sticky server shutdown 的完整 connection 回归现分别覆盖 main、iframe 与 active Dedicated Worker；main/iframe 在第一轮同步循环 marker 后发送后续命令并确认取消后 queued marker 未出现，Worker 先返回独立 `started` 事实再触发 terminal source。Mio accept loop 抽出生产实际使用的确定性批次状态机；脚本结果精确证明命中 256 batch limit 后下一 sweep 跳过 Poll 继续 drain，只有 `WouldBlock` 才重新进入 Poll，且测试不创建真实连接、不接触 256 silent-pending 上限。聚焦 render/no-render 均为 6/6（runs `1caf4a9c-62d1-4467-b911-e66402ca4bdf`、`91614911-e7f3-4a16-944d-091bfd6d1621`）；render CDP 全量 371/371、3 skipped（run `4dd5d821-01f1-4dd7-98c4-b62ec2a8000f`），no-render 排除既有 render-only binary 后 308/308、3 skipped（run `a9e5dfd0-73f7-49f6-a3cb-0a2247ed3077`）。根 release/render 为 2311/2311、4 skipped（run `d7b1445e-1de4-43fb-960c-a249fd834813`）；两种 exact CLI build、固定 benchmark 33/33、官方 Playwright Python 1.60.0 smoke 与完整 37-method profile 均通过。最终 render SHA-256 为 `91931ebd5e54467c809c2d4522a8d35cf8bc96b17bb9136325baf437e3e2092b`，120471088 bytes。200 轮交替 discovery 探针中，基线/候选 median 为 1.762/1.514ms，p95 为 2.031/1.805ms，median 比率 0.859。Astra light 独立终审为 0 blocker、0 major、0 minor。

该切片不把 slot 归零外推成 Worker thread/lease 已 join：`WorkerInstance::drop` 会 terminate/Stop，但不 join，直接 owner-alive Worker cancellation 证据仍来自既有精确测试。main/iframe 的后续命令在 active marker 后发送并等待 20ms，没有独立 server admission barrier；它证明后续命令没有执行，不宣称精确入队竞态资格。Mio 纯状态机也不是 OS backlog burst 或 admission 前 TCP/kernel/container capacity 资格；真实 writer 两项只在本机平台和 active Worker 下运行，不外推为跨平台或 writer source x 三 realm 全矩阵。上述容量、平台、精确 barrier、直接 Worker lease 与剩余 input qualification 仍待完成，OB-034 保持未关闭。

进展（2026-09-22，server admission 与 Worker terminal-boundary 资格切片）：私有 connection I/O test policy 新增 per-server 被动 observer；只有真实 WebSocket reader 完成合法 `CdpRequest` 解帧且 `inbound::Sender::send` 成功后才回报 request ID，overflow/关闭失败路径不会误报。outer I/O task cancellation、sticky server shutdown 以及真实 TCP writer I/O/timeout 的 main/iframe 用例均在 active 同步循环后发送 id=3，并等待该精确 admission barrier 再触发终止；取消后 queued marker 仍必须不存在，不再以固定 20ms sleep 推断已入队。每轮 direct/full-server observer 独立创建，不使用进程全局状态，也不改变生产消息排序。

同一 connection-local `ExecutionCancellation` 现在以固定两次原子操作记录 Dedicated Worker thread closure 生命周期。registration 比 Worker runtime/lease 生命周期更外层，并在既有 completion handoff 之后析构；server 回归先要求计数为 1，再在终止后要求归零，因此直接证明 runtime 与 `WorkerLease` 已释放、completion send 已尝试且 closure 到达 terminal return boundary，不再把 connection slot 归零冒充 Worker 证据，也不宣称 OS thread join。真实 loopback TCP writer 的 write-half I/O failure 与 backpressure timeout 已在本机分别覆盖 main、iframe、active Worker；main/iframe 先排入 8 MiB 合法响应制造阻塞，再进入对应 realm 并等待 id=3 admission，Worker 以 `postMessage` 与线程计数证明已进入。每格仍核对归一化 terminal reason、新产生的完整 writer 日志、socket close、slot 回收及 Worker 计数归零。修复 reviewer 指出的等待竞态后，聚焦 render/no-render 均为 **5/5**（runs `f7b25746-13fc-4ac6-8711-a9ec88f7ba13`、`d49e1e5a-9876-4bd7-9243-25c487833e8a`）；render CDP **371/371**、3 skipped（run `37b79f7e-ad15-48da-a42c-f9f62d6cbeec`），no-render 排除既有 render-only binary 后 **308/308**、3 skipped（run `a21a55a9-05ad-4bd2-b800-02201138d56b`），根 release/render **2311/2311**、4 skipped（run `01fb85ca-4e2a-4bfc-bca2-c22efdc5eb82`）。两种 exact CLI build、固定 benchmark **33/33**、官方 Playwright Python 1.60.0 smoke 与完整 37-method profile 均通过；最终 render SHA-256 为 `ea23d498afe9988536ed1860012a5c4e550e9c89e0fd6fad1a73a0947dcecee7`，120491552 bytes。首次 benchmark 漏传必填 persona 的 **0/33** 配置失败完整保留，不计作产品结果。Astra light 发现并推动修复 owner-alive 测试只等待 lease、未等待 thread guard 的析构竞态；复核为 0 blocker、0 major、0 minor。该本机矩阵不外推到其他产品平台；实际 OS backlog burst、admission 前 TCP/kernel/container capacity、总 RSS 与剩余 input qualification 仍未完成，OB-034 保持未关闭。

进展（2026-09-22，单 worker 实际 OS listen backlog 资格切片，实施 `9f559ea`）：新增标准库工具 `tools/unblocked/cdp_capacity.py`。工具启动指定 release binary，完成 discovery readiness 后记录基线，再向整个单 worker 进程组发送 `SIGSTOP` 并等待内核确认 stopped，之后同时释放完整 `/json/version` 请求。此时进程不能调用 `accept`，所以成功建立的连接只能停在真实 kernel listen queue，不经过 256 条 accepted silent-pending；工具同时要求服务进程数字 FD 计数不增长、目标 listener 的 completed queue 达到同一快照报告的 maximum、未建立项只能是 connect 阶段 Timeout/ETIMEDOUT，不能把 EMFILE、线程启动、发送或本地资源错误冒充 backlog 压力。`SIGCONT` 后每个已发送请求都必须收到完整 200/Content-Length/JSON；队列和 FD 恢复基线后，新 WebSocket 还必须完成 101、raw `Browser.getVersion` 往返和 masked Close 后无错误 EOF。

所有 client request/response、connect/final result、host command stdout/stderr、listener/FD/thread/RSS snapshot、WebSocket payload/frame、server stdout/stderr、异常 traceback 与 SHA-256 都按原始字节保存在 caller-owned 且预先不存在的目录，不脱敏、不截断、不删字段。失败收尾先关闭所有 owned socket、补齐空 response 和最终 result，再恢复 stopped 进程并有界 SIGTERM/SIGKILL。Astra light 三轮审核先发现并推动修复无界 barrier、伪 pressure、Linux 错 listener、失败结果丢失、Darwin FD 口径、截断恢复响应，以及 `ThreadPoolExecutor.submit` 已入队后抛错仍会由旧 worker 执行的证据遗漏；connect/drain 最终都在 executor 结束后从独立加锁 side channel 收集实际结果。定向回归 **14/14**，完整 `tools/unblocked` unittest **66/66**，终审为 0 blocker、0 major、0 minor。
...
进展（2026-09-22，单 worker WebSocket handoff saturation 发布资格切片，实施 `bb1dbd4`）：新增确定性真实 server receive-gate 回归，固定断言 handoff channel capacity 为 128；receiver gate 保持关闭时排入精确 128 个 authorized upgrade，第 129 个必须取得 byte-exact 503/`X-Obscura-Reason: ws-handoff-saturated`，test observer 不得升至 129。测试在 gate 仍关闭时触发 shutdown，并用同一个总 deadline 等待 server 与 128 个 client 全部收尾。该回归不新增生产 hook，只沿用既有 `cfg(test)` policy。

新增 `tools/unblocked/cdp_ws_handoff_capacity.py` 及 19 项定向 unittest。发布工具限定 Darwin IPv4 loopback、精确单 worker、`max-connections=512`、每轮 256 个 socket、3 轮；先发送缺最后 `\r\n\r\n` 的完整 WebSocket head，以 listener completed queue 0、服务端 ESTABLISHED 精确 256、FD 增量至少 256、线程采样和进程存活形成两次稳定 barrier。随后向整个进程组发送 `SIGSTOP`，等待 OS 确认 stopped 且 barrier 不变，再向全部 socket 写入 terminator 并 `SIGCONT`。每条连接只能是完整 101 或唯一完整 503/`ws-handoff-saturated`；每个 101 都发送唯一 masked `Browser.getVersion`、验证 matching result、发送 masked Close 并读到 EOF。每轮必须精确恢复到基线 FD/thread/listen queue/ESTABLISHED，并通过独立 HTTP 和 WebSocket/CDP probe；最后 SIGTERM 要求 returncode 0、process group gone、无 forced kill。`SIGSTOP` 只是外部资格同步，不是产品 hook。

最终提交 `bb1dbd4cdc769545b43abba77a71dce78b01665e` 的 render binary 版本为 `0.1.0-dev+bb1dbd4`，SHA-256 `f28e503665a900f83ebd929bbb1f98078cf1ca559a5d0f7fb05975952825ec7d`，120799200 bytes。最终 `/private/tmp/ob034-ws-handoff-final.tSD284/evidence/` status passed、failures 空；manifest SHA-256 `02e6cf97697ba90c570cfc18ac04ce718eed59716e50282fea8eac2bc2e877c1`，6916483 bytes，8638 个登记 artifact 的 bytes/SHA-256 全部独立复核通过。三轮分别为 167/89、175/81、176/80 个 101/503，全部 518 个 101 都完成 matching CDP 往返；每轮 barrier/stopped 为 FD 272、ESTABLISHED 256、listener completed 0，每轮 recovery 为 FD 16、threads 12、ESTABLISHED/listener completed 0。stderr 原始 31000 bytes，精确 250 条 `WS handoff channel full (128)` warning，与 89+81+80 个 503 一致。首次未冻结调度的 `/private/tmp/ob034-ws-handoff-final.CRldKp/evidence/` 三轮均为 256 个 101，按 exit 2/`not-qualified` 完整保留，不能当作通过。

工具定向 **19/19**、完整 unblocked **133/133**；聚焦 render/no-render 各 **1/1**；render CDP **385/385**、3 skipped，no-render 有效集合 **322/322**、3 skipped、1 binary skipped；release/render 全工作区 **2332/2332**、4 skipped。两种 exact CLI build、固定 benchmark `2340bbb9aea6b8812ff20b7f29113c7c1f9a4b6e` obstacle **33/33**、官方 Playwright Python 1.60.0 smoke 与完整 **37-method** profile 通过；Playwright 原始目录 `/private/tmp/ob034-ws-handoff-playwright.PhtGpC/`。Astra light 首审 0 blocker、2 major、2 minor，推动修复 stopped 状态清理、send-failure raw wire salvage、IPv4 loopback 参数约束和 exact-128 pin；终审 0 blocker、0 major、0 minor。所有命令、request/response/frame 字节、server stream、snapshot、result、traceback 与 hash 原样保留，不脱敏、不截断、不删字段。发布工具不直接读取内部 queue occupancy；唯一 503、精确 warning 数量与恢复证明生产拒绝分支，exact 128 由 Rust gate 证明。结论不外推非 Darwin、multi-worker、container、总 process/FD/RSS/V8/socket-buffer 或剩余 input qualification，OB-034 仍未关闭。

进展（2026-09-22，单 worker WebSocket shared admission 切片，实施 `ada9a69`）：`--max-connections` 现由 authorized WebSocket handoff 与 active processor 共用一个 RAII admission budget。完整且授权的 upgrade request 在进入有界 handoff channel 前取得 permit；permit 随 queued envelope 进入 LocalSet，并由 connection slot 持有到 Page/V8/persistence 与 processor 清理完成。共享上限耗尽时返回完整 503/`X-Obscura-Reason: max-connections`；handoff channel full 或 receiver closed 时返回完整 503/`ws-handoff-saturated`，并在拒绝结束后释放 permit。shutdown 先关闭 receiver、释放仍 queued 的 envelope，再等待 active processor drain。原 active count 保留为 shutdown/idle trim 的生命周期计数，不再充当 admission authority；silent/incomplete HTTP request head 仍使用独立数量与 TTL。

新增确定性真实 loopback Rust gate，覆盖 queued handoff 已占共享 slot、handoff full/closed 的完整 503 与 permit 回收、queued client reset、shutdown queue drain，以及 active processor 清理前 permit 不释放。新增 `tools/unblocked/cdp_ws_capacity.py`，在 Darwin 单 worker、`max-connections=1` 下运行两轮 active/reject/recover：held WS 完成 101 与 raw `Browser.getVersion`，超额 WS 完整 503，held WS 在拒绝后仍可执行命令，主动 Close 后 listen queue、数字 FD 和 thread count 回到基线；最后 active WS 存在时 SIGTERM，要求 clean EOF、returncode 0 且无 forced kill。全部 HTTP request/response、WS frame/payload、host command stdout/stderr、process stream、snapshot、traceback 与 hash 原样保留，不脱敏、不截断、不删字段。

最终 committed evidence 位于 `/private/tmp/ob034-ws-capacity-final.XXvMow/evidence/`，manifest SHA-256 `00679f9def1549ce74f9167ac515a790995df904e55bfda24cb12e2b82d7ea48`，142 个登记 artifact 的 bytes/SHA-256 全部校验通过；status passed、failures 空，两次 recovery 均为 16 FD、12 threads、listen queue 0/128，SIGTERM returncode 0。最终提交 `ada9a69ba33aa28cb98e6cccd8a85bdd9e27194f` 的 render binary 版本为 `0.1.0-dev+ada9a69`，SHA-256 `be2fbf71ea4e4da5d1c1d480ca49589fafda33512f3c71f4e406cd9d375f40cf`，120763696 bytes。工具定向 unittest **12/12**、完整 unblocked unittest **92/92**；聚焦 render/no-render 各 **7/7**（runs `31a36263-d05e-40bf-ac70-5027e6b7b14d`、`290927a0-a2d9-4366-8f89-fba028ec3e7e`）；render CDP **377/377**、3 skipped（run `cd4c545c-f65b-4c38-9cc6-726bbc58f006`），no-render 排除既有 render-only binary 后 **314/314**、3 skipped（run `a6713caf-f5ba-40fb-9740-8a5d9175d353`）；release/render 全工作区 **2324/2324**、4 skipped（run `731021cc-0b61-4c26-8a3a-265a04438e8e`）。两种 exact CLI build、固定 benchmark obstacle **33/33**、官方 Playwright Python 1.60.0 smoke 与完整 **37-method** profile 均通过；Playwright 原始目录 `/private/tmp/ob034-ws-admission-playwright.nPxkn2/`。

Astra light 首审发现并推动修复工具在 read exception 时丢 partial wire、以及 matching-id error response 被误判成功两项问题；终审为 0 blocker、0 major、0 minor。两次 FileExistsError 工具失败分别完整保存在 `/private/tmp/ob034-ws-capacity.C4oTQY/evidence/` 与 `/private/tmp/ob034-ws-capacity.QXRnYK/evidence/`；提交后首次证据 `/private/tmp/ob034-ws-capacity-committed.r438or/evidence/` 使用 Cargo 复用的旧版本 `9f33502`，故保留但不作为最终资格。首轮 obstacle 产品结果虽为 33/33，但外层 zsh 包装误用只读变量 `status` 而退出 1，日志 `/private/tmp/ob034-ws-admission-obstacle.LBwXyi/` 保留；修正包装后的 `/private/tmp/ob034-ws-admission-obstacle-final.xoe87S/` 为 33/33 且退出 0。发布工具稳定覆盖 active shared cap；handoff saturation 由 Rust 真实 loopback gate 确定性覆盖，不宣称发布工具制造了该内部状态。结论不外推 Linux、Windows、container、multi-worker、总 process/FD/RSS/V8/socket-buffer 上限；silent-pending 仍需独立资格，OB-034 保持未关闭。

进展（2026-09-22，accepted silent/incomplete request-head 切片，实施 `6968a9d`）：单 worker accept thread 以 Mio READABLE event reactor 和最早 deadline heap 替换每毫秒扫描最多 256 条 socket 的旧循环。基础 accepted-incomplete budget 为 256，另设固定 16 条、100ms classification reserve，hard total 为 272；基础预算饱和时，已完整到达的 HTTP/WS request head 仍可分类。reserve 到期或 hard cap 返回完整 503/`X-Obscura-Reason: max-pending-request-heads`，半包到 10 秒 TTL 返回完整 408/`request-head-timeout`，全静默连接到期只 close 并记录，不向 wire 伪造 HTTP bytes。已读取 prefix 原样保留；handshake 只看到 `\r\n\r\n` 为止，101 write 完成后才解锁同一 TCP write 中的 frame tail。shutdown 直接丢弃全部 pending，不等待 TTL；deadline heap 对 stale entry 在 `(256+16)*4=1088` 后 compact，固定峰值不随吞吐增长。

确定性 Rust 回归覆盖 4096-byte 431、RST 释放、partial 408/silent EOF、基础 cap 下 HTTP/WS 公平性、observer 证明的 272 hard total、shutdown drain，以及单次 TCP write 同时携带 upgrade head 与 masked `Browser.getVersion id=91` frame 的成功往返。最终工具定向 **22/22**、完整 unblocked **114/114**；聚焦 render/no-render 各 **7/7**，render CDP **384/384**、3 skipped，无 render 有效集合 **321/321**、3 skipped。无 render 的原始完整命令仍含既有 render-only binary 的 6 个 `INPUT_UNSUPPORTED_WITHOUT_RENDER` 失败，结果为 284 passed、37 not run，故不报告为全绿。release/render 全工作区首轮 **2330 passed、1 failed、4 skipped**，唯一失败是既有 MCP `test_wait_for_slow_queued_navigation_uses_the_overall_deadline`；未改源码的精确重放 **1/1** 和完整复跑 **2331/2331**、4 skipped，不把重跑称为修复。

两种 exact CLI release build、固定 benchmark `2340bbb9aea6b8812ff20b7f29113c7c1f9a4b6e` obstacle **33/33**、官方 Playwright Python 1.60.0 smoke 与完整 **37-method** profile 均通过。最终 render binary 为 `0.1.0-dev+6968a9d`，SHA-256 `5eb589e3a158825c53c129847596e61b5b98af1cdad15dc7fab5170d1a12da84`，120799200 bytes。真实 Darwin 24.6.0 arm64、IPv4 loopback、单 worker 证据目录 `/private/tmp/ob034-silent-pending-final.iAT6Kw/evidence/` 的 manifest SHA-256 为 `54310f40c6987d75c1489d6b0cc90b2bb5dd976bf14d58710e229c3ffb50daf1`，5893 个登记 artifact 全部通过 bytes/hash 复核。listener completed queue 为 0 且服务端 ESTABLISHED 精确为 256；该状态 HTTP 8/8 为 200、WS 4/4 完成命令，17/17 overflow 为 503；半包补全为 200，半包 TTL 为 408，256 条零字节连接约 10.000 至 10.030 秒均为空 wire EOF。恢复后 queue/ESTABLISHED 归零、FD 16、threads 12；最终 256 条 held 连接随 SIGTERM 全部空 wire EOF，进程 returncode 0、process group gone、无 forced kill。

Astra light 首审为 0 blocker、6 major、2 minor，推动修复缺失导入、HTTP/空集合验证、稳定恢复验证、TTL 计时、异常清理、stale heap、工具资格范围、失败记录和 first-observed 时点等问题；最终代码、工具与拒绝 helper 微小增量复审均为 0 blocker、0 major、0 minor。工具保留每条 request/response/wire/socket/address、host command stdout/stderr、server stream、traceback 和 hash，不脱敏、不截断、不删字段。真实工具没有资格化 16 条 reserve 同时占满或 272 hard-total 状态，后者由 Rust observer test 确定性覆盖；也不资格化 Linux、Windows、container、multi-worker、总 RSS/FD/CPU/V8/socket-buffer 上限或 portable CPU limit，OB-034 保持未关闭。
