...
进展（2026-09-23，network-idle truthfulness 子切片）：Page/frame/Worker 共用 generation-scoped `NetworkActivityTracker`；`networkidle0` 与 `networkidle2` 按 0/2 active requests 及真实 500ms quiet window 判定。旧 document 的迟到请求 start/terminal 不污染新代。等待超时返回 `NetworkIdleTimeout` 而不伪报 idle；CDP 不再将 DCL/Load 直接投影成 `networkIdle`，commit 后超时仍先同步真实 loader/context 与实际到达的 lifecycle，再返回错误。历史 204/205 导航保留旧文档/loader，并完整投影请求、response、`ERR_ABORTED` terminal 与 `frameStoppedLoading`。

定向 release/render 回归 **4/4**，工具 unittest **140/140**；release/render 全工作区降低 nextest 并发至 2 后 **2389/2389**、4 skipped。默认并发两轮的同一 MCP loopback fixture 连接失败与 **1/1** 精确重放均保留，低并发全量通过；不将重跑称为源码修复。no-render/render exact CLI build、network-idle 五场官方 Playwright smoke、常规 smoke + **37-method** profile、三项 migration gates 及固定 benchmark obstacle **33/33** 通过。Chrome **152.0.7977.85** 同 fixture 的 network-idle/timeouts/204 abort 对照通过；静态 idle 509.9ms、delayed 2016.4ms、never timeout 1004.5ms、document.open 960.0ms、204 abort 3.2ms。该 Chrome 对照不声称 document.open 后完整 DOM 语义相同。Astra light 终审 **0 blocker/0 major/0 minor/0 nit**。

baseline/candidate 以相同 persona、fixture、视口与 capture path 交错运行三轮。静态 idle 中位数为 **601.9ms/600.2ms**，在约 ±10% 噪声内，且候选未扩大相对 Chrome 的既有约 92ms 差值；含 1.5s fetch 的 delayed case 为 **512.7ms/2024.2ms**，差值是等待真实 response 加 quiet window，接近 Chrome 的 2016.4ms；never case 从基线 false success 改为候选与 Chrome 一致的 timeout，204 从基线约 4005ms timeout 改为候选与 Chrome 一致的约 5.6ms abort。基线失败 smoke 是预期反例并完整留档。所有 raw Cargo/client/protocol/fixture 字节及失败记录位于 `/private/tmp/ob021-network-idle-final-build/`。该子切片不关闭 OB-021。
