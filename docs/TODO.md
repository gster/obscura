...
三层逻辑 payload 预算不是总 RSS 上限；真实 writer I/O failure、writer timeout、server shutdown 和 I/O task cancellation 与 active main/iframe/Worker 的组合仍未完全资格化，admission 前 TCP/kernel 分配、容器 capacity、Tungstenite/TCP、上游 observation queue 和响应正文也未纳入该预算，因此 OB-034 保持未关闭。
