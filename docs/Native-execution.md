...
| 传输与其他容量 | 私有 64 KiB 行、队列、4 页与 capture 计数上限 | CDP 每连接 inbound reservation（1024 条、128 MiB、64 MiB 单消息、16 MiB 单 frame）、pending event admission（1024 条、128 MiB、80 MiB 单事件）和 outbound reservation（1024 条、128 MiB、80 MiB 单消息、单次写 10 秒）；MCP HTTP body/read timeout，截图维度限制 | inbound reservation 覆盖 channel、执行中命令及 navigation deferred；pending event 按完整 envelope 序列化 UTF-8 bytes 精确计数，批量全有或全无；三层任一 overflow 都 sticky close，不通过静默丢弃/截断/脱敏继续服务。`Browser.close` 仍只 flush 已接受 outbound envelope。尚未覆盖 admission 前构造、容器 capacity、Tungstenite/TCP、上游 JS/Worker observation 队列、Host/Origin/auth 及同步 V8 中立即断连；页面/body/计算预算归共享层 |
