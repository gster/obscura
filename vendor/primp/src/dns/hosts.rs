...
mod tests {
    ...
    #[tokio::test]
    async fn explicit_resolver_validates_cached_hosts() {
        use std::sync::atomic::{AtomicUsize, Ordering};
        struct Deny(Arc<AtomicUsize>);
        impl Resolve for Deny {
            fn resolve(&self, _: Name) -> Resolving {
                self.0.fetch_add(1, Ordering::SeqCst);
                Box::pin(async { Err(std::io::Error::new(
                    std::io::ErrorKind::PermissionDenied, "denied by custom resolver").into()) })
            }
        }
        // No system hosts file mutation: preload the process-local cache.
        let mut hosts = HostsMap::new();
        hosts.insert("cached-private.example".into(),
            Arc::from([IpAddr::from([127, 0, 0, 1])]));
        assert!(GLOBAL_HOSTS.set(Arc::new(hosts)).is_ok());
        let server = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        server.set_nonblocking(true).unwrap();
        let calls = Arc::new(AtomicUsize::new(0));
        let client = crate::Client::builder().no_proxy()
            .dns_resolver(Arc::new(Deny(calls.clone())))
            .timeout(std::time::Duration::from_millis(200)).build().unwrap();
        assert!(client.get(format!("http://cached-private.example:{}/",
            server.local_addr().unwrap().port())).send().await.is_err());
