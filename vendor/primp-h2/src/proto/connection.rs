    ...
    B: Buf,
{
    ...
    /// Apply peer settings delivered in TLS ALPS without creating a wire ACK.
    pub(crate) fn apply_alps_settings(&mut self, settings: frame::Settings) -> Result<(), Error> {
        self.inner.streams.apply_remote_settings(&settings, false)?;
        ...
        if let Some(size) = settings.max_frame_size() {
            self.codec.set_max_send_frame_size(size as usize);
        }
        self.inner.settings.note_alps_settings();
        Ok(())
    }
