...
where
    ...
    B: Buf,
{
    /// Apply HTTP/2 SETTINGS carried by TLS ALPS before the first request.
    /// ALPS frames do not receive HTTP/2 wire ACKs. Unknown extension frames
    /// (such as ACCEPT_CH) are left to higher layers.
    pub fn apply_peer_application_settings(&mut self, data: &[u8]) -> Result<(), crate::Error> {
        use crate::frame::{Head, Kind, Settings, HEADER_LEN};

        let invalid = |reason: &str| {
            crate::Error::from_io(std::io::Error::new(std::io::ErrorKind::InvalidData, reason))
        };
        let mut remaining = data;
        let mut settings = Vec::new();
        while !remaining.is_empty() {
            if remaining.len() < HEADER_LEN {
                return Err(invalid("truncated ALPS frame header"));
            }
            let length = u32::from_be_bytes([0, remaining[0], remaining[1], remaining[2]]) as usize;
            if remaining.len() < HEADER_LEN + length {
                return Err(invalid("truncated ALPS frame payload"));
            }
            let head = Head::parse(&remaining[..HEADER_LEN]);
            let payload = &remaining[HEADER_LEN..HEADER_LEN + length];
            match head.kind() {
                Kind::Settings => {
                    let frame = Settings::load(head, payload)
                        .map_err(|_| invalid("invalid ALPS SETTINGS frame"))?;
                    if frame.is_ack() {
                        return Err(invalid("ALPS SETTINGS cannot be an ACK"));
                    }
                    settings.push(frame);
                }
                Kind::Unknown => {}
                _ => return Err(invalid("forbidden HTTP/2 frame in ALPS")),
            }
            remaining = &remaining[HEADER_LEN + length..];
        }
        for frame in settings {
            self.inner.apply_alps_settings(frame).map_err(crate::Error::from)?;
        }
        Ok(())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn alps_settings_apply_before_requests_and_reject_invalid_frames() {
        let (client, _server) = tokio::io::duplex(4096);
        let (sender, mut connection) = Builder::new().handshake::<_, Bytes>(client).await.unwrap();
        assert_ne!(sender.current_max_send_streams(), 7);
        // HTTP/2 SETTINGS_MAX_CONCURRENT_STREAMS=7, carried in ALPS.
        let settings = [0, 0, 6, 4, 0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 7];
        connection.apply_peer_application_settings(&settings).unwrap();
        assert_eq!(sender.current_max_send_streams(), 7);
        assert!(connection.apply_peer_application_settings(&settings[..8]).is_err());
        // SETTINGS ACK has no role in ALPS; the server cannot send one here.
        assert!(connection.apply_peer_application_settings(&[0, 0, 0, 4, 1, 0, 0, 0, 0]).is_err());
